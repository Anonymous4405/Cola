...
class PointRecuder(nn.Module):
    ...
    def forward(self, x):   # [2, 5, 224, 224]
        x = self.proj(x)    # [2, 64, 56, 56]
        x = self.norm(x)    # [2, 64, 56, 56]
        return x
...
def pairwise_cos_sim(x1: torch.Tensor, x2: torch.Tensor):
    ...
    """
    x1 = F.normalize(x1, dim=-1)        # [1024, 4, 32]
    x2 = F.normalize(x2, dim=-1)        # [1024, 49, 32]

    sim = torch.matmul(x1, x2.transpose(-2, -1))        # x1 @ x2.T [1024, 4, 49]
    return sim
...
class Cluster(nn.Module):
    def __init__(self, dim, out_dim, proposal_w=2, proposal_h=2, fold_w=2, fold_h=2, heads=4, head_dim=24,
        ...
        self.sim_alpha = nn.Parameter(torch.ones(1))    # alpha
        self.sim_beta = nn.Parameter(torch.zeros(1))    # beta
        self.centers_proposal = nn.AdaptiveAvgPool2d((proposal_w, proposal_h))
        self.fold_w = fold_w
        self.fold_h = fold_h
        self.return_center = return_center

    def forward(self, x):  # [b,c,w,h]      [2, 64, 56, 56]
        value = self.v(x)       # [2, 8*32, 56, 56]
        x = self.f(x)           # [2, 8*32, 56, 56]
        x = rearrange(x, "b (e c) w h -> (b e) c w h", e=self.heads)    # [2*8, 32, 56, 56]
        value = rearrange(value, "b (e c) w h -> (b e) c w h", e=self.heads)    # [2*8, 32, 56, 56]
        if self.fold_w > 1 and self.fold_h > 1:
            ...
            x = rearrange(x, "b c (f1 w) (f2 h) -> (b f1 f2) c w h", f1=self.fold_w,
                          f2=self.fold_h)  # [bs*blocks,c,ks[0],ks[1]]  [2*8*8*8, 32, 7, 7]
            value = rearrange(value, "b c (f1 w) (f2 h) -> (b f1 f2) c w h", f1=self.fold_w, f2=self.fold_h) # [2*8*8*8, 32, 7, 7]
        b, c, w, h = x.shape
        centers = self.centers_proposal(x)  # [b,c,C_W,C_H], we set M = C_W*C_H and N = w*h     [2*8*8*8, 32, 2, 2]
        value_centers = rearrange(self.centers_proposal(value), 'b c w h -> b (w h) c')  # [b,C_W,C_H,c]    [2*8*8*8, 2*2, 32]
        b, c, ww, hh = centers.shape
        sim = torch.sigmoid(        # sig(alpha*s_i + beta)
            self.sim_beta +
            self.sim_alpha * pairwise_cos_sim(
                centers.reshape(b, c, -1).permute(0, 2, 1),     # [1024, 4, 32]
                x.reshape(b, c, -1).permute(0, 2, 1)            # [1024, 49, 32]
            )
        )  # [B,M,N]
